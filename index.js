...
const randomizeBlockCountCheckbox = document.getElementById(
  "randomizeBlockCount"
);
...
const parseHexColor = (hexColor) =>
  hexColor
    .slice(1)
    .match(/(.{2})/g)
    .map((hex) => parseInt(hex, HEX_F));
const hexToRgb = (hexColor) => `rgb(${parseHexColor(hexColor).join(", ")})`;
const hexWithLeadingZero = (number) =>
  `${number < HEX_F ? "0" : ""}${number.toString(HEX_F)}`;
const rgbToHex = (rgbArray) =>
  rgbArray.reduce((hexColor, hex) => hexColor + hexWithLeadingZero(hex), "#");
...
class Block {
  ...
  divideSize(oldSize) {
    return (
      Math.floor(this.splitPosition * ((1 + oldSize) >> 1)) +
      1 +
      (oldSize >> MIN_DIVIDE_RANGE)
    );
  }
...
function getBlockCount(usersBlockCount) {
  const maxBlockCount = Math.min(
    MAX_BLOCK_COUNT,
    Math.max(MIN_BLOCK_COUNT, usersBlockCount)
  );
  return randomizeBlockCountCheckbox.checked
    ? Math.floor(Math.random() * (maxBlockCount - MIN_BLOCK_COUNT)) +
        MIN_BLOCK_COUNT
    : maxBlockCount;
}
...
function generateMap() {
  ...
  const usersBlockCount = isNaN(parseInt(blockCountInput.value))
    ? MAX_BLOCK_COUNT
    : parseInt(blockCountInput.value);
...
function reorderPalette() {
  const maxPaletteCount = Math.floor(
    paletteHolder.clientHeight / PALETTE_BLOCK_SIZE
  );
...
function handleClick({
  shiftKey,
  ctrlKey,
  target: { id, classList: targetClassList },
}) {
  if (id !== "game-panel__desk") {
    if (!shiftKey && !ctrlKey) {
      deskElement
        .querySelectorAll(".active")
        .forEach(({ classList }) => classList.remove("active"));
    }
